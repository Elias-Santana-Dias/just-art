<?php
    ...
    /******************FIM de verificação de quantidade de paginas**************************/
        ...
        if(empty($estado)){
          ...
          ON p.id_userpost = u.id_usuario WHERE (c.nome LIKE '%".$busca."%' OR u.nome LIKE '%".$busca."%') AND flag = 2 GROUP BY p.id_userpost ORDER BY COUNT(p.id_userpost) DESC,u.contratar ASC, u.nome ASC limit $inicio, $quantidade_pg ";
        }else{
        ...
        //$sql = "SELECT * FROM usuario WHERE (cat_art LIKE '%".$busca."%' OR nome LIKE '%".$busca."%') AND flag = 2 ORDER BY id_usuario DESC limit $inicio, $quantidade_pg";
...
<!-- Caso Seja encontrado algum resultado o SCRIPT abaixo sera Exibido-->
<!-- INICIO da Criação do Design de busca com Estado-->
    ...
    <div class="row">
      <div class="col offset-s3 s6">
        <h6 class="center">Total de Artistas Encontrados: <?php echo $total_cursos; ?>. </h6>
      </div>
    </div>
